...
exports.BicycleTypeService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const bicycle_type_entity_1 = require("./bicycle-type.entity");
const fs = require('fs');
let BicycleTypeService = class BicycleTypeService {
    constructor(bicycleTypeRepo) {
        this.bicycleTypeRepo = bicycleTypeRepo;
        this.fileDirectory = "/media/ubaid/ExtraSpace1/tda-crm/tda-be-bicycle/uploads/";
    }
    async createBicycleType(body, file) {
    ...
    async updatedBicycleType(param, body, file) {
        const b = JSON.parse(JSON.stringify(body));
        if (file) {
            b.logo = file.filename;
        ...
        if (result.affected !== 0) {
            ...
            throw new common_1.HttpException("Bicycle Type Not Found!", common_1.HttpStatus.BAD_REQUEST);
        }
    }
    async findBicycleTypeList(query, body) {
        const queryBuild = await this.bicycleTypeRepo
            .createQueryBuilder("bicycle_type")
    ...
    async findBicycleTypeById(param) {
        const queryBuild = await this.bicycleTypeRepo
            .createQueryBuilder("bicycle_type")
            .where("bicycle_type.id = :id", { id: param.id })
            .getOne();
    ...
    async updatedBicycleTypeActivate(param, body) {
        const result = await this.bicycleTypeRepo.update({ id: param.id }, body).catch(err => {
            throw new common_1.HttpException(err.message, common_1.HttpStatus.BAD_REQUEST);
        });
        if (result.affected !== 0) {
    ...
    async updatedBulkBicycleTypeActivate(body1, body2) {
        const queryBuild = await this.bicycleTypeRepo
            .createQueryBuilder()
            .update(bicycle_type_entity_1.BicycleType)
            .set({
...
BicycleTypeService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(bicycle_type_entity_1.BicycleType)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], BicycleTypeService);
exports.BicycleTypeService = BicycleTypeService;
//# sourceMappingURL=bicycle-type.service.js.map