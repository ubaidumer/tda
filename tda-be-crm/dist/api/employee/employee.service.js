...
let EmployeeService = class EmployeeService {
    ...
    async findEmployeeList(query, body) {
    ...
    async findEmployeeById(param) {
        ...
        if (queryBuild !== undefined) {
            ...
            throw new common_1.HttpException("Employee Not Found!", common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async updateEmployeeStatus(param, body) {
    ...
    async updatedEmployee(param, body) {
        const result = await this.employeeRepo.update({ id: param.id }, body).catch(err => {
            throw new common_1.HttpException(err.message, common_1.HttpStatus.BAD_REQUEST);
        });
        if (result.affected !== 0) {
    ...
    async updateEmployeeBulkStatus(body1, body2) {
        const queryBuild = await this.employeeRepo
            .createQueryBuilder()
