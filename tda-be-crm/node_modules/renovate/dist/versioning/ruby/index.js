...
const getSatisfyingVersion = (versions, range) => (0, ruby_semver_1.maxSatisfying)(versions.map(vtrim), vtrim(range));
const minSatisfyingVersion = (versions, range) => (0, ruby_semver_1.minSatisfying)(versions.map(vtrim), vtrim(range));
const getNewValue = ({ currentValue, rangeStrategy, currentVersion, newVersion, }) => {
    ...
    if ((0, regex_1.regEx)(/^('|")/).exec(currentValue)) {
        const delimiter = currentValue[0];
        return newValue
            .split(',')
            .map((element) => element.replace((0, regex_1.regEx)(`^(?<whitespace>\\s*)`), `$<whitespace>${delimiter}`) // TODO #12071
        )
            .map((element) => element.replace(/(?<whitespace>\s*)$/, `${delimiter}$<whitespace>`) // TODO #12071 #12070 adds ' at front when re2 is used
        )
            .join(',');
    }
    return newValue;
