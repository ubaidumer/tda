...
function getNewValue({ currentValue, rangeStrategy, currentVersion, newVersion, }) {
    if (['replace', 'update-lockfile'].includes(rangeStrategy)) {
        if ((0, regex_1.regEx)(/~>\s*0\.\d+/).test(currentValue) &&
            npm_1.api.getMajor(newVersion) === 0) {
            const testFullVersion = (0, regex_1.regEx)(/(~>\s*0\.)(\d+)\.\d$/);
            let replaceValue = '';
            if (testFullVersion.test(currentValue)) {
                replaceValue = `$<prefix>${npm_1.api.getMinor(newVersion)}.0`;
            }
            else {
                replaceValue = `$<prefix>${npm_1.api.getMinor(newVersion)}$<suffix>`;
            }
            return currentValue.replace((0, regex_1.regEx)(`(?<prefix>~>\\s*0\\.)\\d+(?<suffix>.*)$`), // TODO #12071
            replaceValue);
        }
        // handle special ~> 1.2 case
        if ((0, regex_1.regEx)(/(~>\s*)\d+\.\d+$/).test(currentValue)) {
            return currentValue.replace((0, regex_1.regEx)(`(?<prefix>~>\\s*)\\d+\\.\\d+$`), // TODO #12071
            `$<prefix>${npm_1.api.getMajor(newVersion)}.0`);
        }
    }
