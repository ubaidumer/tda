...
exports.descendingRange = exports.ascendingRange = exports.upperBound = exports.lowerBound = exports.inclusiveBound = exports.exactVersion = exports.matchVersion = exports.versionGroup = void 0;
const regex_1 = require("../../util/regex");
// - Replace single \ -> double \
exports.versionGroup = '([0-9a-zA-Z_]+(?:[.-][0-9a-zA-Z_]+)*)';
exports.matchVersion = (0, regex_1.regEx)(`^(?<version>${exports.versionGroup})$`); /* Match a version number (e.g. 1.0.0) */
exports.exactVersion = (0, regex_1.regEx)(`^(?<exact_version>==(?<exact_version_group>${exports.versionGroup})?)$`); /* Match an exact version number (e.g. ==1.0.0) */
...
exports.inclusiveBound = (0, regex_1.regEx)(`^(?<inclusive_bound>(?<inclusive_lower_version>${exports.versionGroup})?\\.\\.(?<inclusive_upper_version>${exports.versionGroup})?)$`); /* Match an inclusive bound (e.g. 1.0.0..2.0.0) */
// Add ? after |\\+) in order to match >=1.15
exports.lowerBound = new RegExp(// TODO #12872 named backreference
`^(?<lower_bound>(?<lower_bound_prefix>>|>=)?(?<lower_version>${exports.versionGroup})?(\\k<lower_bound_prefix>|\\+)?)$`); /* Match a lower bound (e.g. 1.0.0+) */
exports.upperBound = new RegExp(// TODO #12872  lookahead
`^(?<upper_bound>(?<upper_bound_prefix><(?=${exports.versionGroup})|<=)?(?<upper_version>${exports.versionGroup})?)$`); /* Match an upper bound (e.g. <=1.0.0) */
// Add ,? to match >=7,<9 (otherwise it just matches >=7<9)
exports.ascendingRange = new RegExp(// TODO #12872  named backreference
`^(?<range_asc>(?<range_lower_asc>(?<range_lower_asc_prefix>>|>=)?(?<range_lower_asc_version>${exports.versionGroup})?(\\k<range_lower_asc_prefix>|\\+)?),?(?<range_upper_asc>(\\k<range_lower_asc_version>,?|)(?<range_upper_asc_prefix><(?=${exports.versionGroup})|<=)(?<range_upper_asc_version>${exports.versionGroup})?))$`); /* Match a range in ascending order (e.g. 1.0.0+<2.0.0) */
// Add , to match <9,>=7 (otherwise it just matches <9>=7)
exports.descendingRange = new RegExp(// TODO #12872  named backreference
`^(?<range_desc>(?<range_upper_desc>(?<range_upper_desc_prefix><|<=)?(?<range_upper_desc_version>${exports.versionGroup})?(\\k<range_upper_desc_prefix>|\\+)?),(?<range_lower_desc>(\\k<range_upper_desc_version>,|)(?<range_lower_desc_prefix><(?=${exports.versionGroup})|>=?)(?<range_lower_desc_version>${exports.versionGroup})?))$`); /* Match a range in descending order (e.g. <=2.0.0,1.0.0+) */
