...
function rez2npm(input) {
    ...
    if (pattern_1.ascendingRange.test(input)) {
        const match = pattern_1.ascendingRange.exec(input);
    ...
    if (pattern_1.descendingRange.test(input)) {
        const match = pattern_1.descendingRange.exec(input);
...
function pep4402rezInclusiveBound(input) {
    return input
        .split(',')
        .map((v) => v.trim().replace((0, regex_1.regEx)(/[<>=]/g), '')) // TODO #12071
        .join('..');
}
