...
const helper = (0, tslib_1.__importStar)(require("./gitea-helper"));
...
function toRenovatePR(data) {
    ...
    const createdBy = (_f = data.user) === null || _f === void 0 ? void 0 : _f.username;
    ...
    return {
        ...
        hasAssignees: !!(((_g = data.assignee) === null || _g === void 0 ? void 0 : _g.login) || is_1.default.nonEmptyArray(data.assignees)),
    };
}
...
const platform = {
    ...
    async initRepo({ repository, cloneSubmodules, }) {
        let repo;
        config = {};
    ...
    async ensureIssue({ title, reuseTitle, body: content, labels: labelNames, shouldReOpen, once, }) {
        ...
        try {
            ...
            if (issues.length) {
                ...
                const existingIssue = await helper.updateIssue(config.repository, activeIssue.number, {
                    body,
                    title,
                    state: shouldReOpen
                        ? 'open'
                ...
                const existingLabelIds = ((_a = existingIssue.labels) !== null && _a !== void 0 ? _a : []).map((label) => label.id);
                if (labels &&
                    (labels.length !== existingLabelIds.length ||
                        labels.filter((labelId) => !existingLabelIds.includes(labelId))
                            .length !== 0)) {
                    await helper.updateIssueLabels(config.repository, activeIssue.number, {
                        labels,
                    });
                }
                return 'updated';
    ...
    async addAssignees(number, assignees) {
        logger_1.logger.debug(`Updating assignees '${assignees === null || assignees === void 0 ? void 0 : assignees.join(', ')}' on Issue #${number}`);
        await helper.updateIssue(config.repository, number, {
            assignees,
        });
    },
    async addReviewers(number, reviewers) {
        logger_1.logger.debug(`Adding reviewers '${reviewers === null || reviewers === void 0 ? void 0 : reviewers.join(', ')}' to #${number}`);
        if ((0, semver_1.lt)(defaults.version, '1.14.0')) {
    ...
    massageMarkdown(prBody) {
        return (0, pr_body_1.smartTruncate)((0, utils_1.smartLinks)(prBody), 1000000);
    },
