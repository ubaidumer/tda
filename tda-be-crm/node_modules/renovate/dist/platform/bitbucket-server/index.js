...
const defaults = {
    hostType: "bitbucket-server" /* BitbucketServer */,
};
...
async function initRepo({ repository, cloneSubmodules, ignorePrAuthor, }) {
    ...
    try {
        ...
        let cloneUrl = (_a = info.links.clone) === null || _a === void 0 ? void 0 : _a.find(({ name }) => name === 'http');
        if (!cloneUrl) {
            // Http access might be disabled, try to find ssh url in this case
            cloneUrl = (_b = info.links.clone) === null || _b === void 0 ? void 0 : _b.find(({ name }) => name === 'ssh');
        }
        let gitUrl;
        if (!cloneUrl) {
        ...
        const { body: updatedPr } = await bitbucketServerHttp.putJson(`./rest/api/1.0/projects/${config.projectKey}/repos/${config.repositorySlug}/pull-requests/${prNo}`, {
            body: {
                ...
                reviewers: pr.reviewers
                    .filter((name) => !(bitbucketInvalidReviewers === null || bitbucketInvalidReviewers === void 0 ? void 0 : bitbucketInvalidReviewers.includes(name)))
                    .map((name) => ({ user: { name } })),
            },
        });
...
function massageMarkdown(input) {
    logger_1.logger.debug(`massageMarkdown(${input.split('\n')[0]})`);
