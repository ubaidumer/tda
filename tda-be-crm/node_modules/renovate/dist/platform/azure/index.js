...
const azureApi = (0, tslib_1.__importStar)(require("./azure-got-wrapper"));
const azureHelper = (0, tslib_1.__importStar)(require("./azure-helper"));
const types_2 = require("./types");
const util_1 = require("./util");
let config = {};
const defaults = {
    hostType: "azure" /* Azure */,
};
...
async function getRawFile(fileName, repoName, branchOrTag) {
    ...
    const buf = await azureApiGit.getItemContent(repoId, fileName, undefined, undefined, undefined, undefined, undefined, undefined, branchOrTag ? versionDescriptor : undefined);
    const str = await (0, util_1.streamToString)(buf);
    return str;
}
...
async function initRepo({ repository, cloneSubmodules, }) {
    ...
    config.defaultMergeMethod = await azureHelper.getMergeMethod(repo.id, names.project, null, defaultBranch);
    config.mergeMethods = {};
...
async function getStatusCheck(branchName) {
    ...
    return azureApiGit.getStatuses(branch.commit.commitId, config.repoId, undefined, undefined, undefined, true);
}
const azureToRenovateStatusMapping = {
    [GitInterfaces_1.GitStatusState.Succeeded]: types_1.BranchStatus.green,
    [GitInterfaces_1.GitStatusState.NotApplicable]: types_1.BranchStatus.green,
    [GitInterfaces_1.GitStatusState.NotSet]: types_1.BranchStatus.yellow,
    [GitInterfaces_1.GitStatusState.Pending]: types_1.BranchStatus.yellow,
    [GitInterfaces_1.GitStatusState.Error]: types_1.BranchStatus.red,
    [GitInterfaces_1.GitStatusState.Failed]: types_1.BranchStatus.red,
};
...
async function getBranchStatus(branchName) {
    ...
    const noOfFailures = statuses.filter((status) => status.state === GitInterfaces_1.GitStatusState.Error ||
        status.state === GitInterfaces_1.GitStatusState.Failed).length;
    if (noOfFailures) {
        return types_1.BranchStatus.red;
    }
    const noOfPending = statuses.filter((status) => status.state === GitInterfaces_1.GitStatusState.NotSet ||
        status.state === GitInterfaces_1.GitStatusState.Pending).length;
    if (noOfPending) {
        return types_1.BranchStatus.yellow;
    }
...
async function createPr({ sourceBranch, targetBranch, prTitle: title, prBody: body, labels, draftPR = false, platformOptions, }) {
    ...
    const workItemRefs = [
        {
            id: (_a = platformOptions === null || platformOptions === void 0 ? void 0 : platformOptions.azureWorkItemId) === null || _a === void 0 ? void 0 : _a.toString(),
        },
    ];
    ...
    if (platformOptions === null || platformOptions === void 0 ? void 0 : platformOptions.usePlatformAutomerge) {
    ...
    if (platformOptions === null || platformOptions === void 0 ? void 0 : platformOptions.azureAutoApprove) {
...
async function updatePr({ number: prNo, prTitle: title, prBody: body, state, }) {
    ...
    if (state === types_1.PrState.Open) {
        await azureApiGit.updatePullRequest({ status: GitInterfaces_1.PullRequestStatus.Active }, config.repoId, prNo);
    }
    else if (state === types_1.PrState.Closed) {
        objToUpdate.status = GitInterfaces_1.PullRequestStatus.Abandoned;
    }
...
const renovateToAzureStatusMapping = {
    [types_1.BranchStatus.green]: [GitInterfaces_1.GitStatusState.Succeeded],
    [types_1.BranchStatus.green]: GitInterfaces_1.GitStatusState.Succeeded,
    [types_1.BranchStatus.yellow]: GitInterfaces_1.GitStatusState.Pending,
    [types_1.BranchStatus.red]: GitInterfaces_1.GitStatusState.Failed,
};
...
async function mergePr({ branchName, id: pullRequestId, }) {
    ...
    const mergeMethod = (_a = config.mergeMethods[pr.targetRefName]) !== null && _a !== void 0 ? _a : (config.mergeMethods[pr.targetRefName] = await azureHelper.getMergeMethod(config.repoId, config.project, pr.targetRefName, config.defaultBranch));
    const objToUpdate = {
    ...
    logger_1.logger.trace(`Updating PR ${pullRequestId} to status ${GitInterfaces_1.PullRequestStatus.Completed} (${GitInterfaces_1.PullRequestStatus[GitInterfaces_1.PullRequestStatus.Completed]}) with lastMergeSourceCommit ${pr.lastMergeSourceCommit.commitId} using mergeStrategy ${mergeMethod} (${GitInterfaces_1.GitPullRequestMergeStrategy[mergeMethod]})`);
    try {
        const response = await azureApiGit.updatePullRequest(objToUpdate, config.repoId, pullRequestId);
        let retries = 0;
        let isClosed = response.status === GitInterfaces_1.PullRequestStatus.Completed;
        while (!isClosed && retries < 5) {
            ...
            isClosed = pr.status === GitInterfaces_1.PullRequestStatus.Completed;
        }
        if (!isClosed) {
            logger_1.logger.warn({ pullRequestId, status: pr.status }, `Expected PR to have status ${GitInterfaces_1.PullRequestStatus[GitInterfaces_1.PullRequestStatus.Completed]}, however it is ${GitInterfaces_1.PullRequestStatus[pr.status]}.`);
        }
        return true;
    }
    catch (err) {
