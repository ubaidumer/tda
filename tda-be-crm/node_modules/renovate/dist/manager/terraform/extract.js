...
const dependencyBlockExtractionRegex = (0, regex_1.regEx)(/^\s*(?<type>[a-z_]+)\s+("(?<lookupName>[^"]+)"\s+)?("(?<terraformName>[^"]+)"\s+)?{\s*$/);
            ...
            if (terraformDependency) {
                ...
                switch (tfDepType) {
                    ...
                    case common_1.TerraformDependencyTypes.provider: {
                        result = (0, providers_1.extractTerraformProvider)(lineNumber, lines, terraformDependency.groups.lookupName);
                        break;
                    }
                    case common_1.TerraformDependencyTypes.module: {
                        result = (0, modules_1.extractTerraformModule)(lineNumber, lines, terraformDependency.groups.lookupName);
                        break;
                    }
