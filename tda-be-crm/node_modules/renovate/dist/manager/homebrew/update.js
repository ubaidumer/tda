...
function updateUrl(content, oldUrl, newUrl) {
    ...
    while ((0, util_1.removeComments)(newContent) !== testContent) {
        i += 'url'.length;
...
async function updateDependency({ fileContent, upgrade, }) {
    ...
    try {
        ...
        newUrl = `https://github.com/${ownerName}/${repoName}/releases/download/${upgrade.newValue}/${repoName}-${String((0, semver_1.coerce)(upgrade.newValue))}.tar.gz`;
        newSha256 = await (0, hasha_1.fromStream)(http.stream(newUrl), {
            algorithm: 'sha256',
        });
    }
    catch (errOuter) {
