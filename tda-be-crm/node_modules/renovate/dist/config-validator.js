...
(async () => {
    ...
    for (const file of app_strings_1.configFileNames.filter((name) => name !== 'package.json')) {
        try {
            const rawContent = await (0, fs_extra_1.readFile)(file, 'utf8');
            ...
            try {
                ...
                logger_1.logger.info({ err }, `${file} is not valid Renovate config`);
                returnVal = 1;
            }
        }
        catch (err) {
    ...
    try {
        ...
        if (!(0, dequal_1.dequal)(fileConfig, {})) {
            const file = (_a = process.env.RENOVATE_CONFIG_FILE) !== null && _a !== void 0 ? _a : 'config.js';
            logger_1.logger.info(`Validating ${file}`);
            try {
