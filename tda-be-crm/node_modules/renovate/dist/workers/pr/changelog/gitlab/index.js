...
function getRepoId(repository) {
    return repository.replace((0, regex_1.regEx)(/\//g), '%2f');
}
async function getTags(endpoint, repository) {
    var _a;
    logger_1.logger.trace('gitlab.getTags()');
    const url = `${(0, url_1.ensureTrailingSlash)(endpoint)}projects/${getRepoId(repository)}/repository/tags?per_page=100`;
    try {
...
async function getReleaseNotesMd(repository, apiBaseUrl, sourceDirectory) {
    ...
    const apiPrefix = `${(0, url_1.ensureTrailingSlash)(apiBaseUrl)}projects/${repoid}/repository/`;
...
async function getReleaseList(apiBaseUrl, repository) {
    logger_1.logger.trace('gitlab.getReleaseNotesMd()');
    const repoId = getRepoId(repository);
    const apiUrl = `${(0, url_1.ensureTrailingSlash)(apiBaseUrl)}projects/${repoId}/releases`;
