...
parentKey) {
    ...
    try {
        ...
        for (const [key, val] of Object.entries(config)) {
            if (key === 'pathRules') {
                ...
                if (is_1.default.nonEmptyArray(val) && val.includes('prEditNotification')) {
                    migratedConfig.suppressNotifications =
                        migratedConfig.suppressNotifications.filter((item) => item !== 'prEditNotification');
                }
            }
            ...
            else if (key === 'enabledManagers' && is_1.default.array(val)) {
                // Replace yarn with npm, since yarn actually uses npm as package manager
                migratedConfig.enabledManagers = migratedConfig.enabledManagers.map((element) => (element === 'yarn' ? 'npm' : element));
            }
                ...
                migratedConfig.matchHost || (migratedConfig.matchHost = val);
                ...
                migratedConfig[key] = val.replace((0, regex_1.regEx)(/{{baseDir}}/g), // TODO #12071
                '{{packageFileDir}}');
            }
            else if (is_1.default.string(val) && val.includes('{{depNameShort}}')) {
                migratedConfig[key] = val.replace((0, regex_1.regEx)(/{{depNameShort}}/g), // TODO #12071
                '{{depName}}');
            }
            else if (key === 'gitFs') {
                ...
                if (!is_1.default.string(migratedConfig.npmrc)) {
                    migratedConfig.npmrc = '';
                }
            }
            else if (key === 'branchName' &&
                is_1.default.string(val) &&
                (val === null || val === void 0 ? void 0 : val.includes('{{managerBranchPrefix}}'))) {
                migratedConfig.branchName = val.replace('{{managerBranchPrefix}}', '{{additionalBranchPrefix}}');
            }
                ...
                for (let i = 0; i < presets.length; i += 1) {
                    ...
                    if (is_1.default.string(preset)) {
                        let newPreset = common_1.removedPresets[preset];
                        if (newPreset !== undefined) {
                            presets[i] = newPreset;
                        }
                        newPreset = migratePresets === null || migratePresets === void 0 ? void 0 : migratePresets[preset];
                        if (newPreset !== undefined) {
                            presets[i] = newPreset;
                        }
                    }
                ...
                for (let i = 0; i < schedulesLength; i += 1) {
                    if (schedules[i].includes(' and ') &&
                        ...
                        schedules[i].replace((0, regex_1.regEx)(/( \d?\d)((a|p)m)/g), '$1:00$2') // TODO #12071
                        ).schedules[0];
                ...
                if (is_1.default.array(migratedConfig === null || migratedConfig === void 0 ? void 0 : migratedConfig[key])) {
                    const newArray = [];
            ...
            if (is_1.default.string(migratedConfig[key])) {
                for (const [from, to] of Object.entries(migratedTemplates)) {
                    migratedConfig[key] = migratedConfig[key].replace((0, regex_1.regEx)(from, 'g'), // TODO #12071
                    to);
                }
            }
        }
